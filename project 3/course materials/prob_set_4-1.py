    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for field in FIELDS:
                ...
                if line[field] == "NULL":
                    #print 'found null in %s' % field
                    dict[field_name] = None
                else:
                    dict[field_name] = line[field].strip()
                    
            # we can clean this row here, since all columns have been read
            dict['label'] = clean_label(dict['label'])
            
            name = dict['name']
            if (name is None or name == "NULL" or not name.isalnum()):
                dict['name'] = dict['label']
                
            if (dict['synonym'] is not None):
                syn = re.sub('{|}|(\* *)', '', dict['synonym'])
                dict['synonym'] = syn.split('|')
                #print 'aaaaa %s' % dict['synonym']
                
            cn = {}
            cn['kingdom'] = dict['kingdom']
            cn['phylum'] = dict['phylum']
            cn['class'] = dict['class']
            cn['family'] = dict['family']
            cn['order'] = dict['order']
            cn['genus'] = dict['genus']
            del dict['kingdom']
            del dict['phylum']
